...
키 값의 크기를 16B 자식 노드의 크기를 12B라 하면
한페이지에 저장할 수 있는 키의 수는 16*1024/(32+12) = 585개 저장할 수 있다.
키 값의 크기가 32B로 늘어나면 한 페이지에 저장할 수 있는 갯수가 372개로 줄어든다.</br>
...
- 순서대로 읽다 리프노드 끝에 다르면 리프노드간의 링크를 이용해 다른 리프노드를 찾아 다시 스캔.
- 요구된 모든 데이터를 읽으면 반환.
...
2. 인덱스 스캔. 1번의 위치부터 필요한 만큼 인덱스를 읽음.
...
다음의 복합 인덱스에서 각각의 칼럼이 내림 차순과 오름 차순이 혼합된 경우, 내림차순 인덱스로만 해결가능하다.
...
3) **가용성과 효율성 판단**</br>
   B-Tree 특성상 다음 조건에서는 작업 범위 결정 조건으로 사용할 수 없다.</br>
